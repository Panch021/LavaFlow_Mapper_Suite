"""
lavaflow_suite/app.py
=====================
Main Dash application (Global Config + all analysis tabs).

Launch with any of::

    lavaflow-suite                    # console script (pip install .)
    python -m lavaflow_suite
    python lavaflow_mapper_suite.py   # launcher kept at the repo root

Project data (``projects/``, ``examples/`` and ``active_volcano.txt``) are
read from the current working directory, or from ``--workdir``.
"""
import argparse
import dash
...
# Technical modules
from . import firms_download as download_logic
from . import anomalies as anomalies_module
from . import frp_statistics as stats_module
from . import mapper as mapper_module
from . import animation as anim_module
from . import speed as speed_module
from . import report as export_module

# ==========================================
# 0. CONFIGURATION & DATA ENGINE
# ==========================================

GVP_FILE = lfc.gvp_catalogue_path()
if GVP_FILE and os.path.exists(GVP_FILE):
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(prog='lavaflow-suite', description=__doc__.split('\n')[3])
    parser.add_argument('--workdir', default=None,
                        help='folder holding projects/, examples/ and active_volcano.txt')
    parser.add_argument('--port', type=int, default=9050, help='HTTP port (default 9050)')
    ...
    app.run(debug=args.debug, host=args.host, port=args.port, dev_tools_hot_reload=False)
