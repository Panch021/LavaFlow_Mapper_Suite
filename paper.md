...
The core of the method is the spatial and thermal filtering of the FIRMS detections. Raw VIIRS detections spread well beyond the lava-flow field because of the large footprint of off-nadir pixels and the presence of low-intensity anomalies; retaining only pixels with a small along-track size (track) and a Fire Radiative Power (FRP) above a user-defined threshold isolates the anomalies that correspond to active lava (\autoref{fig:filter}).

![Thermal anomalies detected by VIIRS (Suomi-NPP and NOAA-20) during the 2018 lower East Rift Zone eruption of Kīlauea, coloured by date, before (left) and after (right) applying the track (ground sampling distance) and FRP filters implemented in LavaFlow Mapper. The black outline is the official lava-flow map of the eruption [@Zoeller2020; @Dietterich2021].\label{fig:filter}](Abstract_a-01.png){ width=100% }

The package includes eight core modules:

1. **Global Configuration**: defines the volcano of interest (directly from the Global Volcanism Program catalogue), vent coordinates, analysis period, processing thresholds and optional reference layers (radius, shapefile and waypoints). Optional layers are disabled for new projects so that maps are always centred on the vent.
...
3. **Anomalies Count**: calculates weekly and monthly frequencies of thermal anomalies per sensor, labels each bar with its total, and summarises the period (last day, last week, last month, peak week and peak month). Charts and summary are derived from a single aggregation, so the reported totals always match the plotted bars.
...
5. **LavaFlow Mapper**: generates preliminary maps of thermal anomalies using spatial and thermal filtering criteria and produces FRP and distance-to-vent time series. The mean, 95th percentile and maximum of both variables are recomputed interactively for the time window displayed on screen.

6. **LavaFlow Propagation**: animates the evolution of thermal anomalies through time and renders the animation as a portrait MP4 video (map, time bar with automatically scaled date ticks, and FRP/distance series) using the map extent, basemap, reference layers and playback speed selected by the user.

7. **Propagation Speed**: estimates lava-flow propagation rates by tracking the most distal thermal anomalies; maximum distance and maximum/mean speed are updated interactively for the displayed time window, and the reference radius can be shown or hidden.

8. **Export Report**: generates self-contained, responsive HTML reports built with the same figure generators as the interactive modules, so that statistics, interactive behaviour and, optionally, the propagation video are identical to those shown in the application.

All modules share a common helper library for project management, configuration parsing, date handling and waypoint validation, and the interface adapts to the screen size and resolution, allowing the suite to be used on desktop and laptop computers running Windows, macOS or Linux.
